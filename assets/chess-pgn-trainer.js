...
function resumeSavedGame() {
	...
	// Update UI after loading puzzle
...
function initalize() {
	...
	// Fix: Manually trigger PGN loading if a file is already selected (e.g., after resume)
...
function loadPuzzle(puzzle) {
	...
	if (puzzle.fen) {
		...
		game.reset(); // Ensure it's start position if no FEN
	}
...
function startTest() {
	...
	if (!puzzleset || puzzleset.length === 0) {
		console.warn('No puzzleset loaded');
		return;
	}
	...
	// Load first puzzle if not already loaded
	if (increment === 0 && !game.history().length) {
		loadPuzzle(puzzleset[PuzzleOrder[increment]]);
	}
